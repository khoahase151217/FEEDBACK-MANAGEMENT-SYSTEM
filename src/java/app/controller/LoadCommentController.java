...
public class LoadCommentController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<FeedbackDTO> listComment = dao.showListFeedbackResponseNext(Integer.parseInt(amount));
            for (FeedbackDTO comment : listComment) {
                out.println("<a href=\"ShowFeedbackResponeDetailForManagerController?feedback_response_id=" + comment.getFeedbackID() + "&style_list=comment\">\n"
                        + "                                                                            <div class=\"pipe-comment-item\">\n"
                        + "                                                                                <div class=\"pipe-item-heading\">\n"
                        + "                                                                                    <div class=\"pipe-item-title-wrapper\">\n"
                        + "                                                                                        <h3 class=\"pipe-item-title\">\n"
                        + "                                                                                            Feedback " + comment.getFeedbackID() + "\n"
                        + "                                                                                        </h3>\n"
                        + "                                                                                    </div>\n"
