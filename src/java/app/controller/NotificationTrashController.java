 ...
 * @author ASUS
 */
@WebServlet(name = "NotificationTrashController", urlPatterns = {"/NotificationTrashController"})
public class NotificationTrashController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int check2 = dao.countForNotificationTrash();
            if (check2 > check) {
                notification = check2 - check;
                list = dao.getListFeedbackForNotificationTrash(notification);
            }
            ...
            for (FeedbackDTO feedback : list) {
                out.println("<div class=\"notification-item\" onclick=\"handleReloadPage()\">\n"
                        ...
                        + "                                                        <strong>Denied because of misInformation</strong>\n"
                        + "                                                    </p>\n"
                        + "                                                </div>\n"
                        + "                                            </div>");
            }
